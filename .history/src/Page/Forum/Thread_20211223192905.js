...
const Thread = () => {
  ...
  onLoad();}
    ...
    const showRep= threadList.map((threadList) =>  <div key={threadList.id}>  <br></br> {threadList.reply} <br></br><p className="">{threadList.email}</p>Level: <strong>{threadList.level}</strong></div>)
