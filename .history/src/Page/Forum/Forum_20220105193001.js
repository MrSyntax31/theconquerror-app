    ...
    getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
     ...
     charactersLength));
       ...
       return result;
        ...
        <div className="heading" style={{cursor:"pointer"}}><ul> <li onClick={ handler}  data-id ={topic.id}>{topic.title}<br></br><strong>Description:</strong> {topic.desc} <br></br><strong>Tags: </strong>{topic.tags}</li></ul></div></div> <Badge bg="primary">{topic.case_status}</Badge>))
