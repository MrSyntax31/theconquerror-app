...
const Thread = () => {
  ...
  onLoad();}
    ...
    const showRep= threadList.map((threadList) =>  <div key={threadList.id}>  <br></br> {threadList.reply} <br></br><OverlayTrigger overlay={<Tooltip id="tooltip-disabled">Tooltip!</Tooltip>}><p className="text-primary d-inline-block" data-email={threadList.email} onClick={askReport} style={{cursor:"pointer"}}>{threadList.email}</p></OverlayTrigger> <bre Level: <strong>{threadList.level}</strong></div> )
