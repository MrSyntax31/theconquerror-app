...
import { getFirestore, doc, onSnapshot, addDoc, collection, query, orderBy , deleteDoc} from '@firebase/firestore';
...
const Thread = () => {
  ...
  async function DeletePost(e) {
   ...
   const postid = JSON.stringify(e.target.getAttribute("data-id"))
    const postemail = topic.created_by

   if(user.email === postemail)
   {
    await deleteDoc(doc(forumdb, "topics", postid)).then(()=>{
  ...
  modal.style.display = "none";
    ...
    return <div className="commentSection">
        ...
        <Dropdown>
            ...
            <Dropdown.Menu>
                <Dropdown.Item data-id={topic.id} onClick={DeletePost}>Delete</Dropdown.Item>
...
{ topic.sampcodeimg &&  <img className="mx-auto d-block img-fluid mt-3 mb-3 image-preview image-preview-js" style={{width: '25%', height: 'auto'}} src={topic.sampcodeimg} alt={topic.title} onClick={enlargeImage}/>  }

<div id="ShowIMGModal" className="ShowIMGModal" >
 
<span className="close" onClick={close}>X</span>

<img className="mx-auto d-block img-fluid mt-3 mb-3 image-preview image-preview-js"  style={{width: '25%', height: 'auto'}} src={topic.sampcodeimg} alt="EnlargedImage" onClick={enlargeImage}/>
