...
import { onSnapshot,collection,getFirestore, query, orderBy, doc, getDoc} from 'firebase/firestore';
...
const Lessons = () => {
  ...
  //Lessons Instructions Collection
  const progid = sessionStorage.getItem('getProg')
  ...
  useEffect(() => {
    ...
    async function fetchProg(){
      ...
      if (progid === null)
      {
        ...
        const docRef = doc(forumdb, "courses", progid);
        const docSnap = await getDoc(docRef);
        
        if (docSnap.exists()) {
          setProgcourse(docSnap.data())
              ...
              <Container fluid="md" style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                ...
                <Card className="mt-5">
                  <Tabs defaultActiveKey="profile" id="uncontrolled-tab-example" className="mb-3 mt-3">
                    <Tab eventKey="profile" title="Overview" className="mt-2 mb-5">
                        <p className="mt-2 mb-5 text-justify">{courses.introduction}</p>
