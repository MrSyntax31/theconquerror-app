...
const Lessons = () => {
  ...
  const enroll =  function(e){
    ...
    const difficulty = parseInt(e.target.getAttribute("data-difficulty"),10)
      ...
      if (currentUser === null)
      {
        if (window.swal({type: 'error', icon: 'error', title: 'Oops', text: 'You need to be logged in to continue!'}) <Button variant="primary"> Explore Computer Programming</Button>) {
          // Save it!
          <Button variant="primary"> Explore Computer Programming</Button>
         //history.push("/login")
        } else {
          //
        }
