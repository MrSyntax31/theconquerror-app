...
const LessonsContent = () => {
  ...
  const [lessonid, setLessonData] = useState(JSON.  (sessionStorage.getItem('getLesson')));
  ...
  const enroll = async function(e){
    ...
    const listkey = e.target.getAttribute("data-id");
      ...
      {
        const id = JSON.Stringify(sessionStorage.setItem('getLesson',listkey))
        setLessonData(id);
        console.log("wews")
        fetchLesson();
      }
...
const showCourse = courses1.map((courses1) => (  
  <div key={courses1.id} className="card mb-5">
    <div className="single-feature wow fadeInUp m-2 p-1" data-wow-delay=".4s">
      ...
      <Button onClick={enroll}  data-id={courses1.id} >Get Started</Button>
    </div>
  </div>
))
