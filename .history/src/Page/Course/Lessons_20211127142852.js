...
const Lessons = () => {
  ...
  useEffect(
    () => {
      ...
      const collectionRef = collection(forumdb, "courses", "progcourse");
  ...
  const enroll = async function(e){
    ...
    const displayCourse = progcourse.map((progcourse) => (
      <div className="col-md-4">
          <p>{progcourse.introduction}</p>
      </div>
    ))
                ...
                <Card className="mt-5">
                  <Tabs defaultActiveKey="profile" id="uncontrolled-tab-example" className="mb-3 mt-3">
                    <Tab eventKey="profile" title="Overview" className="mt-2 mb-5">
                        <p className="mt-2 mb-5 text-justify">{progcourse.introduction}</p>
