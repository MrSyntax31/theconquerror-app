...
useEffect(
    ...
    //Data for Gender
      ...
      function NumberofMale(){
  
        const collectionRef = query(collection(admindb, "userdata"), where("Gender", "==", "Male"));
      const q = query(collectionRef);
      onSnapshot(q, (snapshot) =>
      setMaleData(snapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })))
    ...
    function NumberOfFemale(){

      const collectionRef = query(collection(admindb, "userdata"), where("Gender", "==", "Female"));
    const q = query(collectionRef);
    onSnapshot(q, (snapshot) =>
    setFemaleData(snapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })))
  ...
  function NumberofStudents(){
    ...
    const collectionRef = query(collection(admindb, "userdata"), where("Occupation", "==", "Student"));
  const q = query(collectionRef);
  onSnapshot(q, (snapshot) =>
...
const [instructor, setInstructorData] = useState([]);

function NumberofInstructors(){

  const collectionRef = query(collection(admindb, "userdata"), where("Occupation", "==", "Instructor"));
const q = query(collectionRef);
onSnapshot(q, (snapshot) =>
setInstructorData(snapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })))
...
function InstiCount(){
  ...
  const collectionRef = query(collection(admindb, "userdata"), where("Institution", "==", "Polytechnic University of the Philippines"));
const q = query(collectionRef);
onSnapshot(q, (snapshot) =>
setInsti(snapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })))
    ...
    return (
        <>
            ...
            <Helmet>
                ...
                <meta name="description" content="Welcome to ConquError Admin page. Check out the documentation and reports of the latest updates within the ssytem." />
            </Helmet>
